...
const preview: DeletionPreviewResponse = {
  ...
  retained: { containers: ['full-id'], owned_volumes: [{ name: 'owned', configured_in: 'draft', exists: false }], external_volumes: [{ name: 'external', configured_in: 'draft', exists: true }], binds: [{ source: '/srv/data', readonly: true, configured_in: 'draft', exists: true }], networks: [{ name: 'network', bridge_name: null, kind: 'external', aliases: ['app'], configured_in: 'draft', exists: false }] },
