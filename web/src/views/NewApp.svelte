...
<section class="panel"><h2>{$t('Quick deploy')}</h2><p class="muted">{$t('Versioned built-in presets create regular drafts and never run arbitrary Compose.')}</p><a class="button-link" href="#/apps/new/postgresql">PostgreSQL</a> <a class="button-link" href="#/apps/new/pgadmin">pgAdmin</a></section></main>
