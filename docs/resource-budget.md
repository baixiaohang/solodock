...
两份报告共同验证 idle、streams、部署/回滚与受限 metadata 场景，但不等价于腾讯云真实宿主测量，也不会把 Docker 峰值混入 SoloDock RSS。目标主机上线前仍需在维护窗口按同一 workload 复测；`MemoryHigh=256M` 是 soft pressure signal，不是 kill limit，若目标宿主的 control-plane 常态或峰值显著高于 CI 基线再调整。
