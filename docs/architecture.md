...
Filesystem scans are non-destructive at startup and runtime: verified loaders, catalog refresh, and reconciliation cannot delete a concurrent writer's temporary artifact, an unreferenced revision, or any other cleanup candidate. Destructive recovery is limited to exact ledger-owned finalizers whose authenticated artifact and durable proof agree. Administrator storage cleanup uses a separately confirmed plan and atomically detaches each selected artifact into its authenticated tombstone before that finalizer can remove the payload.
