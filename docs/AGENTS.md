...
- Changes to architecture, behavior, APIs, configuration, storage layout, deployment/rollback semantics, operations, security, or test guardrails must update both language versions in the same Pull Request. A translation-only wording correction may change only the Chinese file when it does not alter meaning.
