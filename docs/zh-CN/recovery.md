...
helper 会在解包前拒绝绝对路径、`..`、hard link、特殊文件和非预期顶层，并在任何 owner/mode 修改前校验解出的全部 link：config 必须是非 symlink 的普通文件，整个 staging 中唯一允许的 symlink 是 SoloDock 自有的 canonical `apps/<app UUID>/{active,pending} -> releases/<release UUID>`。在修改 owner/mode 前，generation-bound Rust validator 还会要求解出的 config 声明官方 `/var/lib/solodock` state 与 `/run/solodock` runtime 路径；来自自定义路径源码部署的 archive 必须显式迁移，不会被部分发布。Helper 会解析已安装的 `solodock` 系统账户，在不跟随 symlink 的前提下把整个私有 staging tree 映射到该精确 UID/GID，再以服务身份运行版本绑定的 validator。validator 会把 canonical managed leaf 的 legacy `0400`/`0600` 规范化为 `0444`，并严格复核 owner/mode、link boundary、HMAC、config revision，以及 staging 中仍按安装路径身份生成的 canonical Compose。archive 的 owner 字段不会被信任或保留；意外的 `0644` mode、symlink 或特殊文件不会被自动修复。发布会锚定到已检查的父目录身份，并采用原子 no-replace 操作。不要在线覆盖现有 state。将当前 `/var/lib/solodock` 原子改名为可恢复备份，再把验证后的完整 state/config 切入：目录与普通控制面文件分别保持 `0700/0600`，`files/{public,secret}` direct leaf 保持 `0444`，最后启动并检查 journal、`/healthz` 和认证 system health。

binary、config 和 state 必须来自兼容的一组备份。SQLite migration 是 forward-only；迁移后仅回滚 binary 不安全。

## 更新失败

`solodock-update` 默认跟随当前版本绑定 `INSTALL_MANIFEST` 记录的 channel，只有管理员显式传入 `--channel` 才换轨。它会在 apply 前验证 stable Release identity 或 main CI identity、provenance attestation、source commit、package version、完整 package identity 与 checksum。随后由已验证的新 binary 在停服务或备份前校验已安装 packaged layout 并输出配置的 loopback probe URL；custom path 或畸形输出会保持服务与 package 不变。preflight 失败不会改变已安装 package、服务或 state，并会清理临时下载；之后 stable 与 main 共用同一个 apply 路径。package/helper 或 channel identity 变化但 binary bytes 相同时，只安装这些资产并检查 health，不停服务也不调用 binary；binary 变化时则共用停止服务、离线备份、安装、启动、`/healthz` 与 `/favicon.svg` 路径。即使 GitHub 的 Latest Release 事实意外回退，已安装 stable manifest 也会阻止低于当前 SemVer 的自动降级。
...
镜像删除使用独立持久 plan，item 状态为 `planned`、`started`、`removed` 和 `retained`。手动操作必须显式重试原 request/key；自动保留操作使用策略授权在后台恢复；继续前服务端重新验证 plan、preview 绑定、item ledger、当前 artifact 恢复 inventory 和 Docker 引用。`started` item 的 exact ID 已不存在时，无须重复 remove 即可确认；新增引用或冲突的镜像保留。Remove 响应丢失、删除后 inspect 失败、item/response commit 失败在重新核验完成前仍是结果未知。
...
自动保留在启动和周期轮次恢复已发布的精确计划，不依赖浏览器 session。应用/策略快照及计划 hash 必须与 `automatic_cleanup_authorizations` 一致，结果记录与 durable item 状态一致后才能 finalize。关闭策略会停止新选择并保留 canonical 项；已经 detach 或 remove 的项仍完成精确记账。备份须将 SQLite 策略/授权表与 filesystem state 一并保留。镜像 inventory 错误使镜像阶段受阻并可重试，不回退成功部署。
