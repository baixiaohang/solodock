...
> [English](../deployments.md)（权威版本） · 简体中文
...
管理员确认或应用策略授权的存储清理可以移除旧 release，同时永久保留其 deployment 历史。该详情返回 `safe_release_id: null`、不提供 rollback action，并带有 `ROLLBACK_ARTIFACT_CLEANED` warning。持久化的 cleaned-release 记录会把这种明确清理与普通 release 缺失或损坏区分开；直接回滚请求也会被拒绝。

## Artifact 清理后的镜像

Artifact 阶段不删除 Docker 镜像；自动保留在 artifact finalize 后执行单独的镜像阶段。独立镜像预览从已验证 artifact plan 与 detached item ledger 推导来源，仅在 artifact finalize 完成后才允许作为候选。新增的保留 release 或 container 引用会阻止镜像删除。Image apply 先取得 catalog guard、按 UUID 顺序取得全部当前 app guard 和共享 Compose guard，再 fresh recheck。回滚与部署历史语义不变：未启用的应用额外保留三个回滚 release；已启用应用保留 active 及最近成功的不同 release，达到配置总数，并额外保护 pending/恢复引用；已明确清理的 artifact 即使镜像仍存在，也不能回滚。
