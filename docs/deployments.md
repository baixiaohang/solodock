...
Every replacement, compensation restore, and manual rollback has a stop-to-start bind guard. SoloDock confirms the exact predecessor or candidate writer is exited, then freshly resolves the immutable target release's bind sources, checks allowlist/symlink/device/inode facts, and inventories other live applications before the next Compose start-like action. A changed path or read-write ancestor conflict leaves the deployment interrupted or needing attention; no candidate or predecessor is started through the failed guard.
...
An administrator-confirmed or application-policy-authorized storage cleanup may remove an old release while retaining its deployment history. Such a detail has `safe_release_id: null`, no rollback action, and warning `ROLLBACK_ARTIFACT_CLEANED`. The durable cleaned-release record distinguishes this deliberate loss from ordinary missing or corrupt release state; direct rollback requests are rejected as well.

## Images after artifact cleanup

The artifact phase does not remove Docker images. Automatic retention runs a separate image phase after artifact finalization. The independent image preview derives provenance from the verified artifact plan and detached item ledger, only after artifact finalization has completed. A newly retained release or container reference vetoes image deletion. Image apply takes the catalog guard, every current application guard in UUID order, and the shared Compose guard before its fresh recheck. Rollback and deployment history remain unchanged: disabled applications retain three additional rollback releases; enabled applications retain the active release plus the latest distinct successful releases up to their configured total, with pending/recovery references additionally protected, and an explicitly cleaned artifact remains unavailable for rollback even if its image is still present.
