...
The helper rejects absolute paths, `..`, hard links, special files, and unexpected top-level entries before extraction. It then validates every extracted link before any ownership or mode change: the config must be a non-symlink regular file, and the only allowed symlinks anywhere in staging are SoloDock-owned canonical `apps/<app UUID>/{active,pending} -> releases/<release UUID>` links. It resolves the installed `solodock` system account, maps the complete private staging tree to that exact UID/GID without following symlinks, then runs the version-bound validator as the service identity. The validator normalizes legacy `0400`/`0600` modes on canonical managed leaves to `0444` and verifies owner/mode, link boundaries, HMACs, config revisions, and canonical Compose. Archive owner fields are never trusted or preserved; unexpected `0644` modes, symlinks, and special files are not repaired. Publication is an atomic no-replace operation anchored to the checked parent directory identity. Never overwrite live state. Atomically rename the current `/var/lib/solodock` to a recoverable backup before switching in the complete validated state/config. Keep directories at `0700`, ordinary control-plane files at `0600`, and direct `files/{public,secret}` leaves at `0444`, then start the service and inspect the journal, `/healthz`, and authenticated system health.
