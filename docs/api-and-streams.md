...
The draft editor allows continued input while saving. A confirmed save advances the current editing session to the response’s `app.config_revision` and updates the saved Secret/file baseline without overwriting later input. Submitted sensitive values are cleared only when the corresponding input still matches; subsequent saves project keep/replace/delete against the confirmed revision. A failed follow-up read retains that revision and the input while marking application facts as stale. A read of a different, externally modified revision still requires conflict handling; an unknown mutation result never advances the baseline. Public bulk text, including invalid lines, its editing mode, and validation errors survive application tab switches within the editing session. Preflight success and error responses apply only to the unchanged editing session that submitted them. Save and preflight validate the current text, and explicit draft reload or a new editing session resets it.

Draft requests accept optional `environment.order`, an array listing every active public and secret key exactly once (excluding deleted secrets). Draft responses return it as `environment_order` when present. It is persisted in config metadata and covered by the config integrity hash; an order change creates a new configuration revision. Omitted order retains the legacy canonical ordering. Existing revisions need no migration and keep their original hashes.
...
pgAdmin v1 uses `preset_id: "pgadmin"`, `preset_schema_version: 1`, and `variables: {"email": "admin@example.com", "password": "<16–256 bytes>", "host_port": 5050}`. Its public descriptor includes `image` and `default_host_port`; PostgreSQL-specific defaults remain exclusive to PostgreSQL descriptors. The preset fixes the image to `dpage/pgadmin4:9.17`, publishes only `127.0.0.1:<host_port>:5050/tcp`, and enables service discovery. Passwords use the existing managed-secret pipeline and are never returned. Unknown variables, invalid email/port/password, and unsupported preset/schema combinations are rejected.
...
Both cleanup preview endpoints return `409 CLEANUP_RECOVERY_REFERENCE_MISSING` when a required deployment recovery reference is absent. The standard envelope carries a request ID; the corresponding sanitized log includes application/deployment IDs. Other incomplete inventories continue to use `CLEANUP_INVENTORY_INCOMPLETE`. Rejected previews issue no confirmation token and perform no cleanup.
...
Fresh facts differing from the preview produce `CLEANUP_PREVIEW_STALE` before token consumption or any remove. Invalid/expired previews, busy application/Compose guards, and incomplete inventory also have no effect before publication. Consume, exact selected plan/items, and sanitized audit publish in one SQLite transaction. Only exact HTTP 200 with a validated per-image terminal result confirms completion. `completed_with_failures` reports retained items and requires a fresh preview. Unknown transport/daemon/SQLite results keep the same body/key; a published retry cannot be converted to a known rejection by busy guards or another session.

## Application retention policy

`GET /api/v1/apps/{id}/retention` returns `enabled`, `keep_versions`, `revision` and the last pass's `last_checked_at`, `last_status`, `last_error_code`. Missing policy defaults to disabled, three versions and a nil revision. `PUT` on the same route requires `expected_revision`, `enabled`, `keep_versions` (1–100), and `Idempotency-Key`; it retains management session, exact Origin, CSRF, no-store and 16 KiB body boundaries. Stale revision returns `409 RETENTION_REVISION_STALE`; busy application mutation returns `409 APP_BUSY`. Retry an unknown result with the same body/key. Policy, audit and the exact successful replay response commit atomically before waking background cleanup; it does not wait for or imply deletion success.

Pass states are `pending`, `completed`, `partially_retained`, or `blocked`; errors distinguish `APP_BUSY`, `ARTIFACT_CLEANUP_BLOCKED` and `IMAGE_CLEANUP_INVENTORY_INCOMPLETE`. These are cleanup results and never modify deployment status. Existing manual preview/confirmation routes and their idempotency semantics remain available.
