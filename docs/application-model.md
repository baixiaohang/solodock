...
Storage cleanup supports explicit preview-and-confirm operations and opt-in application retention policies. It always protects active and pending releases, the current draft revision, recovery references from `queued`, `running`, `interrupted`, and `needs_attention` deployments, cleanup recovery artifacts, and the application retention set: the legacy three additional rollback releases when automatic retention is off, or the active release plus the latest distinct successful releases up to the configured total when enabled. A preview selects at most 100 of the globally oldest verified, unreferenced releases; config revisions become candidates only when no retained release or draft references them. Known private temporary artifacts use the same typed store inventory. Unknown names, links, types, ownership, modes, signatures, or ledger facts make the entire inventory fail closed.
...
Manual image cleanup requires per-image selection (initially empty) and a separate acknowledgement. Automatic retention selects only cleaned-release identities belonging to its enabled application, after artifact finalization. Only confirmed `cleaned_releases` identities qualify. Every physical retained release, application-deletion tombstone release, and unfinished artifact-cleanup reference protects its images; this includes ordinary historical releases outside the retention target. Running and stopped containers, whether managed or unmanaged, protect their config/manifest identities. Digest, platform, descriptor, recovery proof, or complete inventory failures prevent an executable preview. No container, volume, bind, network, credential, deployment, audit record, or backup is removed.
