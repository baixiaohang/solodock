...
Image cleanup is independent of artifact cleanup and requires per-image selection (initially empty) and a separate acknowledgement. Only confirmed `cleaned_releases` identities qualify. Every physical retained release, application-deletion tombstone release, and unfinished artifact-cleanup reference protects its images; this includes ordinary historical releases outside the three rollback points. Running and stopped containers, whether managed or unmanaged, protect their config/manifest identities. Digest, platform, descriptor, recovery proof, or complete inventory failures prevent an executable preview. No container, volume, bind, network, credential, deployment, audit record, or backup is removed.
