...
Relevant Pull Requests run the full DinD regression against pinned Docker 28.5.2 classic image storage and four `containerd_` scenarios against pinned Docker 29.7.2: descriptor deployment/no-op, pre-marker erroneous-claim cleanup, post-marker replacement preservation, and conservative manual image cleanup. `Extended CI` periodically reruns both daemon baselines. Both daemon modes have backend assertions: the classic job rejects `io.containerd.snapshotter.v1`, while the containerd job must observe it, preventing both jobs from silently using one storage mode. The classic DinD job mounts a dedicated fixture root beneath the workspace into the daemon service at the same absolute path. Managed-file bind sources must be under that root, never a runner-temporary path invisible to the daemon, and must not mount the production Docker socket. Each `containerd_` scenario has a bounded overall deadline; deployment scenarios additionally bound gates and shutdown. Success, failure, panic, or timeout cleans only this scenario's containers, networks, declared volumes, and temporary bind sources by exact application/project/full ID and ownership label. The job timeout is only a last resort.
...
- Password rotation covers Origin, CSRF, session, JSON shape/body limit, shared cooldown, wrong-current HTTP 403, exact cookie expiry, old/new login behavior, and sanitized audit metadata. Failure injection proves the credential hash, every existing session, throttle, and audit roll back together without expiring cookies. Concurrent old-password login cannot survive a successful rotation, and an existing SSE closes on its next heartbeat.
- The Settings password form sends only the current and new password without idempotency or automatic retry; mismatch and policy errors send nothing, deterministic errors stay authenticated, confirmed success returns to login, and ambiguous transport results expose neither password canaries nor a false success claim.
...
## Manual image cleanup gates

The real containerd cleanup E2E has two mandatory phases on an isolated daemon. It keeps a tag-created registry container with its index parent and first requires its selected child to be independently absent: preview must return 409 without creating a token or operation, and all candidate images and resource canaries must survive. The fixture then explicitly pulls the registry and Alpine selected children by exact repository@digest, verifies fresh child inspection and unchanged container identities, and runs the original successful preview/removal assertions. It does not rely on other E2Es preparing those children, skip the negative phase, or replace the registry container with a single-manifest container. Failure of explicit preparation is a test failure. This setup demonstrates complete versus incomplete inventories, not runtime automatic repair or universal containerd compatibility.

The containerd index regression preserves a tag-created container's index, selected child manifest and config ID. Missing child inspection, wrong child platform/descriptor, wrong parent identity and nested-index substitution all reject preview/apply before token consumption or removal. The HTTP adapter also gates index media-type projection separately from manifest identity.

Focused `m3_api image_cleanup::` cases use the production router and real artifact cleanup provenance: all four running/stopped × managed/unmanaged container classes, ordinary retained releases, fresh races, application/Compose guards, invalid selection, incomplete identity/inventory and durable ledger corruption. Actual daemon-effect mocks and SQLite triggers cover pre-remove failure, lost remove response, post-remove inspect failure, progress/response commit failure, audit rollback and explicit restart/retry; only exact selected images can be removed.

`image_cleanup_adapter` uses a loopback HTTP daemon fixture, not host Docker, and checks full unfiltered container enumeration, missing inspection failure, exact full IDs, `force=false`, `noprune=true`, conflict retention and confirmed absence. It runs explicitly with the `docker-e2e` feature in CI. The isolated classic/containerd `manual_image_cleanup` E2E scenarios create unique committed images in a private fixture registry, publish signed releases, clean them through the artifact API, and apply only selected eligible images through the image API. Unselected images, running/stopped unmanaged containers, and volume/network canaries survive; teardown touches only exact recorded fixture resources. These host-Docker tests are CI gates, not part of default local focused validation.

Web tests cover zero requests on mount, both default-false gates, exact body/key retry after network errors, unexpected 204/202 and malformed 200, known stale/partial results, safe local errors and late unmounted responses.
