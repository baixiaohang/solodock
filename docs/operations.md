...
Run `solodock-update` as the administrator account, not through `sudo solodock-update`. The updater uses `sudo` internally for both the config file safety checks and config inspection, because `/etc/solodock` is owned by `solodock:solodock` with mode `0700` and `config.toml` has mode `0600`. Keep these permissions; the administrator does not need direct access to the config directory. Missing configs, symlinks, and non-regular files are rejected before service changes.
...
Use **System settings → Storage cleanup** for SoloDock-owned immutable artifacts. Scan first, inspect the exact release/config/temp list and rollback loss, then acknowledge and apply. A preview is not a lock: apply rechecks all protected facts and refuses a changed plan without deleting anything. Applications without automatic retention retain the legacy three additional recent rollback releases, beyond active/pending/recovery facts. Enabled applications use their configured total successful-version count in both manual and automatic plans. Artifact cleanup never touches workload data or Docker resources. The displayed logical size is an estimate, not guaranteed reclaimed space.

Both cleanup panels distinguish `CLEANUP_RECOVERY_REFERENCE_MISSING`: a deployment still requires a release or configuration revision that is absent. Use the displayed request ID to locate the application/deployment IDs in the service log. Other inventory failures retain their conservative error classification. An old `needs_attention` row can remain visible after its verified application unregistration; history and attention counts are not rewritten by cleanup. See [recovery](recovery.md#completed-application-unregistrations) for legacy reconciliation.

## Per-application container security profiles

An application may select an optional `security_profile`, such as `codex-v1`, in its configuration. Empty/null keeps Docker defaults. This is a name of a **host-installed pair**, not arbitrary Compose security options: SoloDock generates `seccomp=/etc/solodock/security-profiles/<name>/seccomp.json` and `apparmor=solodock-<name>` for that application's single service. Names contain 1–48 lowercase ASCII letters, digits or hyphens, starting with a letter; `unconfined` is rejected. This does not add capabilities, host namespaces, or privileged mode.

The host operator must install and load both policies before deployment and arrange AppArmor loading on boot. Keep profile files root-owned and not writable by managed containers. SoloDock does not upload, edit, install or relax host policies. A missing/invalid profile fails container creation through the existing deployment failure path; there is no fallback to an unconfined container. Compose preview alone cannot prove kernel compatibility: run the application's sandbox smoke test on the target host before promoting the deployment.

Treat names as immutable version identifiers. Never replace the contents of an installed version; install a new pair under a new name when changing policy. Configuration revision schema 4 protects the selected name with the existing integrity hash. Release, retry, recovery and rollback load that same revision. Keep old seccomp files and loaded AppArmor profiles while any running container or retained rollback release references them. Host profile content is operator-managed, not copied into release artifacts; rollback assumes the original named policy is still installed. Reverting the application setting to empty and redeploying restores Docker defaults without changing other applications.

## Manual Docker image cleanup

Known conservative limitation: some normally running multi-architecture containers have a selected child that Docker cannot inspect independently by digest. This blocks the **entire image cleanup preview and automatic image phase**, including unrelated eligible images; it does not stop or impair those applications. SoloDock does not ignore such containers or automatically pull images to repair the inventory. This version does not support every otherwise-valid containerd inventory. The E2E fixture's explicit child preparation is test setup, not a runtime recovery feature.
...
Use the separate **System settings → Docker image cleanup** panel after artifact cleanup. Scan, select individual images, acknowledge, then apply. Manual image selection starts empty. Applications with automatic retention enabled also have a separate policy-authorized background pass; neither path is triggered by a disk threshold. The preview is not a lock. All release and running/stopped container references are rechecked before effects; the daemon additionally enforces non-force conflicts. Docker-reported bytes are an upper estimate, not guaranteed reclaimed space or ownership: shared layers may remain.

Only exact fresh image IDs are removed, with `force=false` and `noprune=true`. In-use or multiple-reference conflicts retain the image; SoloDock never escalates to force/prune or removes parents. Containers, volumes, networks, business data, credentials, backups, deployments and audit history remain outside this operation. An incomplete inventory or more than 4,096 live containers fails closed; each preview allows at most 100 selectable images. Resolve the inventory problem before rescanning; do not bypass it with broad deletion.

Image cleanup scans all cleaned-release history in stable, bounded pages within one database read snapshot. It validates every release and cleanup proof before selecting images, including late-page protection and missing-operation checks. Historical images already absent do not block cleanup, and repeated image identities share Docker observations. Cleanup history and rollback-loss markers remain intact; the live-container inventory limit and the maximum of 100 removals per operation still apply.

## Automatic version retention

Enable **Automatic version cleanup** on an application's overview and choose 1–100 versions (default 3). All applications, including existing installations, start disabled. Enabling authorizes cleanup of existing excess releases and their unused local images. The setting is operational SQLite state, does not create configuration drift, and is not restored by a workload rollback.

The active release occupies one slot; the other slots contain distinct releases ordered by successful deployment completion time (deployment ID breaks ties). Failed attempts, no-ops and repeated deployments of the same release do not consume slots. Without active, retain up to N successful releases. Pending, deployment recovery and cleanup recovery references remain additionally protected; the count is a target, not a forced cap. Different configuration releases remain different versions even when they share an image.

A single background worker wakes on policy changes and deployment completion, scans on startup and every 60 seconds, and coalesces bursts with a five-second pause. Each pass processes at most 100 artifacts and 100 images per application; later passes advance remaining backlog and retry temporary failures. Image batches prioritize never-attempted and least-recently-attempted identities using durable attempt history, so persistent non-force conflicts cannot starve later images; final plans remain sorted by exact image ID. Artifact finalization precedes image selection. Automatic selection excludes temporary files and applications that have not opted in, while retained releases in every application and every running/stopped host container veto image deletion.

The application overview displays the last pass, time, retained items and blocked phase; manual previews remain in System settings. An image inventory error, including an unavailable multi-architecture child, retains images and reports the image phase as blocked until retry. Cleanup failures never change a successful deployment result. Disabling stops new selection; an interrupted exact deletion still completes its durable accounting/finalization, retaining newly protected canonical items. History remains available, but cleaned releases cannot be rolled back. Volumes, bind data and remote registries are never cleaned.
