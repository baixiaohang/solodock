...
[[ -f $unit_source && ! -L $unit_source && -f $config_source && ! -L $config_source ]] || { printf '%s\n' 'package assets are missing or unsafe' >&2; exit 2; }
if [[ -n $destdir ]]; then
...
if [[ -e $target || -L $target ]]; then
  if [[ ! -L $target ]]; then
    printf '%s\n' 'refusing to replace unknown non-symlink binary target' >&2
    exit 1
  fi
  current=$(readlink -- "$target")
  [[ $current == /usr/local/lib/solodock/*/solodock ]] || { printf '%s\n' 'refusing unknown symlink target' >&2; exit 1; }
fi
if [[ -z $destdir ]]; then
