...
impl RegistryResolver {
    ...
    pub async fn resolve(
        ...
        let deadline = tokio::time::timeout(
            ...
            self.resolve_inner(image, platform, credential),
        );
        deadline.await.map_err(|_| RegistryError::Timeout)?
    }
    ...
    async fn resolve_inner(
        ...
        let first = self.fetch(image, &image.tag, None).await?;
        ...
        let response = if first.status() == StatusCode::UNAUTHORIZED {
                ...
                .is_ok_and(|value| value.starts_with("Basic "))
            {
                ...
                bearer = Some(token);
                self.fetch(image, &image.tag, bearer.as_ref().map(SecretValue::expose))
                    .await?
            }
        } else {
            first
