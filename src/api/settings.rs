...
pub async fn update(
    ...
    let settings = if current.revision == payload.expected_revision {
        match store
            .update(payload.expected_revision, &payload.display_timezone)
            .await
        {
            Ok(value) => value,
    ...
    } else if resumed && current.display_timezone == payload.display_timezone {
        current
    } else {
        return finish_error(
            services,
